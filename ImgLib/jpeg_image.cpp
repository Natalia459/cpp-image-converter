...
namespace img_lib {
	...
	void SaveImagelineToScale(std::vector<JSAMPLE>& buffer, int y, const Image& image) {
	...
	bool SaveJPEG(const Path& file, const Image& image) {
		...
		//jpeg_set_quality(&cinfo, 192, TRUE /* limit to baseline-JPEG values */);
		...
		std::vector<JSAMPLE> buffer(row_stride, 255);
		while (cinfo.next_scanline < image.GetHeight()) {
			SaveImagelineToScale(buffer, cinfo.next_scanline, image);
